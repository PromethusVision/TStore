...
class ProductionPreviewProductRepository
    implements ProductRepository, TaxonomyScopedProductRepository {
  const ProductionPreviewProductRepository({
  ...
  Future<Either<String, List<ProductEntity>>> _read(
    ...
    try {
      final raw = await adapter.readProducts(args);
      if (raw is! List) {
        throw const FormatException('Preview product response.');
      }
      final ids = <String>{};
      final products = raw
          .map((item) {
            if (item is! Map) {
              throw const FormatException('Preview product row.');
            }
            ...
            if (row['canonical_category_id'] != product['category_id'] ||
              ...
              throw const FormatException('Preview product mapping mismatch.');
            }
  ...
  Future<Either<String, ProductEntity>> getProductById(String id) async {
    ...
    return result.bind(
      (items) => items.length == 1 && items.single.id == id
          ? Right(items.single)
          : const Left('Ürün önizlemede kullanılamıyor.'),
    );
  }
  ...
  Future<Either<String, List<ProductEntity>>> getProductsByIds(
    ...
    for (final id in ids.toSet()) {
      ...
      if (result.isLeft()) {
        return const Left('Ürün önizleme kapsamı doğrulanamadı.');
      }
