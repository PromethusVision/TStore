...
export async function support({newest=false}={}){
 const path=process.env.W52LC_PROOF_DIR,rel=relative(root,path??root);
 check(isAbsolute(path??'')&&(rel==='..'||rel.startsWith('..'+sep)||isAbsolute(rel)),'EXTERNAL_PRIVATE_PROOF_DIRECTORY');
 check(['w52kb-lcone','w52kb-lctwo'].includes(process.env.W52KB_CONTAINER),'LOCAL_CONTAINER_ALLOWLIST');
 globalThis.fetch=async()=>{throw Error('W52LC_EXTERNAL_NETWORK_FORBIDDEN');};
