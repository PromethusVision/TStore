...
export function parse(argv){
 ...
 const fields=['seal-sha256','sql-sha256','project-ref','psql','psql-sha256','ca','ca-sha256','pg-dump','pg-dump-sha256','pg-restore','pg-restore-sha256','backup','backup-metadata'];
...
export async function run(argv){
 ...
 try{
  options=parse(argv);verify(options.seal_sha256);factory=connectionFactory(options);db=factory.open();
  if(options.operation==='preflight')await readState(db,options.seal_sha256);
  else if(options.operation==='postflight')await readState(db,options.seal_sha256,true);
  else if(options.operation==='deploy')await deploy(db,options,()=>factory.backup());
  else await rollback(db,options);
  result.result='PASS';result.operation=options.operation;
 }catch(e){
  result.safe_error=safeError(e);
  if(options?.write_attempted){
   result.result='STOPPED_REQUIRES_RECONCILIATION';
   if(options.operation==='deploy'&&factory){
    try{if(db)await db.close();db=null;result.reconciliation=await reconcile(factory,options);result.result=result.reconciliation.result;}catch{result.result='STOPPED_REQUIRES_REVIEW';}
   }
  }
 }finally{
