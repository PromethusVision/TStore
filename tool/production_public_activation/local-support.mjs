...
export async function support({newest=false}={}){
 const path=process.env.W52LA_PROOF_DIR,rel=relative(root,path??root);
 check(isAbsolute(path??'')&&(rel==='..'||rel.startsWith('..'+sep)||isAbsolute(rel)),'EXTERNAL_PRIVATE_PROOF_DIRECTORY');
 check(['w52kb-laprepare','w52kb-lanewest','w52kb-laone','w52kb-latwo'].includes(process.env.W52KB_CONTAINER),'LOCAL_CONTAINER_ALLOWLIST');
 globalThis.fetch=async()=>{throw Error('W52LA_EXTERNAL_NETWORK_FORBIDDEN');};
