...
import {check,root,read,hash} from '../live/common.mjs';
export const originalArchive='8d98258430fbb7eeccc8ead4e09efa59189aad84e961bb754fe223f07c3fcbc8';
export const newestArchive='ebbd10a9aa518ebacfb306bda6853647271e78ffa440a0baab0b2c1efabd3183';
export async function support({newest=true}={}){
 const path=process.env.W52LF_PROOF_DIR,rel=relative(root,path??root);
 check(isAbsolute(path??'')&&(rel==='..'||rel.startsWith('..'+sep)||isAbsolute(rel)),'EXTERNAL_PRIVATE_PROOF_DIRECTORY');
 check(['w52kb-lfone','w52kb-lftwo'].includes(process.env.W52KB_CONTAINER),'LOCAL_CONTAINER_ALLOWLIST');
 globalThis.fetch=async()=>{throw Error('W52LE_EXTERNAL_NETWORK_FORBIDDEN');};
 ...
 for(const f of files){
  ...
  if(newest&&f==='restore.mjs')source=once(source,"toc.includes('TOC Entries: 965') && entries.length === 958","toc.includes('TOC Entries: 1117') && entries.length === 1110");
  if(!newest&&f==='baseline.mjs')source=once(source," WHERE version<>'20260916001200'"," WHERE version NOT IN ('20260916001200','20260919001300','20260922001500')");
