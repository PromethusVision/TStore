...
export async function support({newest=false}={}){
 const path=process.env.W52LE_PROOF_DIR,rel=relative(root,path??root);
 check(isAbsolute(path??'')&&(rel==='..'||rel.startsWith('..'+sep)||isAbsolute(rel)),'EXTERNAL_PRIVATE_PROOF_DIRECTORY');
 check(['w52kb-leone','w52kb-letwo'].includes(process.env.W52KB_CONTAINER),'LOCAL_CONTAINER_ALLOWLIST');
