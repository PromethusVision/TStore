...
export function connectionFactory(options){
 ...
 try{
  ...
  Object.assign(environment,{PGHOST:`db.${project}.supabase.co`,PGPORT:'5432',PGDATABASE:'postgres',PGUSER:'postgres',PGSSLMODE:'verify-full',PGSSLROOTCERT:ca,PGCONNECT_TIMEOUT:'15',PGAPPNAME:'w52le-0014-public-activation',PGPASSFILE:'__w52le_no_passfile__',PGOPTIONS:'-c default_transaction_read_only=on'});
  ...
  return {
   ...
   async backup(){
    ...
    check(/Format: CUSTOM/.test(toc)&&/TABLE DATA public products/.test(toc)&&/FUNCTION public production_taxonomy_runtime_v1/.test(toc)&&['read_capabilities','products','listings','shops'].every(n=>toc.includes('FUNCTION public production_public_'+n+'_v1')),'BACKUP_TOC_IDENTITY');
