...
import { check, hash, json, project, stable, validateBackup } from '../production_taxonomy/execution/common.mjs';
...
export const container = process.env.W52KB_CONTAINER;
export const docker = process.env.W52KB_DOCKER;
export const dump = process.env.W52KB_DUMP;
...
export const latestBackupHash = '8d98258430fbb7eeccc8ead4e09efa59189aad84e961bb754fe223f07c3fcbc8';
const approvedBackup = () => [latestBackupHash].includes(hash(readFileSync(dump)));
export function run(args, input, binary = false) {
  check(docker && /^w52kb-[a-z0-9]+$/.test(container ?? ''), 'LOCAL_PARAMETERS');
...
export function guard() {
  ...
  check(state.Config.Labels['esnaftavar.task']==='w52k-b','LOCAL_TASK_CONTAINER');
...
export async function create() {
  ...
  run(['run','-d','--name',container,'--network','none','--label','esnaftavar.task=w52k-b','--user','postgres','--mount',`type=bind,source=${dump},target=/backup/production.dump,readonly`,'--entrypoint','sh',imageDigest,'-c',cmd]);
  let ready=false;
