...
import { run, container, check, sha256, stableJson } from './real_restore_lib.mjs';
...
export class HttpChecks {
  ...
  async start() {
    ...
    run(['exec','-i',container,'sh','-c','umask 077; cat > /tmp/w52hr-postgrest.conf'],Object.entries(config).map(([k,v])=>`${k} = ${JSON.stringify(v)}`).join('\n')+'\n');
    run(['exec','-d',container,'sh','-c','echo $$ > /tmp/w52hr-postgrest.pid; exec /tmp/postgrest /tmp/w52hr-postgrest.conf > /tmp/w52hr-postgrest.log 2>&1']);
    let ready=false;
    for(let i=0;i<15;i++) {
  ...
  stop() {
    run(['exec',container,'sh','-c','if test -f /tmp/w52hr-postgrest.pid; then kill "$(cat /tmp/w52hr-postgrest.pid)"; fi']);
    this.key=null;
  }
}
