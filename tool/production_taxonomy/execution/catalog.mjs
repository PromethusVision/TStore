...
export async function catalog(db) {
  const queries = {
    ...
    // Text relation names otherwise inherit the database's ICU/libc ordering.
    // Preserve every constraint field; fix only the serialization order.
    constraints: `SELECT conrelid::regclass::text AS relation,conname AS name,contype::text AS type,convalidated,pg_get_constraintdef(oid) AS definition FROM pg_constraint WHERE connamespace='public'::regnamespace ORDER BY conrelid::regclass::text COLLATE "C",conname COLLATE "C"`,
