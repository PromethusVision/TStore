...
## Integration validation and binary lineage

Integration base: `d43ee49032ce786612ae10e11c96e8072ae33696`.
Reviewed source: `396ff801a5f0cc969828359fe8bcd5f6c1b05a78`.
Integration branch: `integration/esnaftavar-official-logo`.

- Independently compared the repository PNG with the Product Owner's original:
  byte-identical, matching the SHA-256 above.
- Targeted asset/widget/layout tests: **35 PASS / 0 FAIL / 0 SKIP**.
- Full Flutter suite: **2,198 PASS / 0 FAIL / 6 existing conditional SKIP**;
  skip identities match the integrated baseline. No live-test opt-in was used.
- Analyzer: **PASS**. Native resource wiring, aspect ratio, light/dark resources
  and Android 12 safe-area tests passed. No device or iOS simulator run occurred.
- Reviewed all 15 changed Home goldens: official branding and the resulting
  header/body positioning account for the changes; bottom navigation is identical.
- Diff and secret/PII checks: **PASS**. PNG metadata findings were public C2PA
  certificate-authority contact information; test addresses were existing
  `example.com` fixtures. No secret or personal-data finding remained.

```text
BINARY_AFFECTING_DELTA: YES
EXISTING_1_0_0_PLUS_3_BINARY_EQUIVALENT_TO_FINAL_MAIN: NO
NEW_SIGNED_BUILD_REQUIRED: YES
APK_REBUILT: NO
AAB_REBUILT: NO
...
The earlier frozen APK's physical acceptance remains historical evidence for
that exact binary. A new branded signed build is required from integrated main.
