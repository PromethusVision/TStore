...
Status: implementation in progress. Ledger created before runtime edits.
...
| --- | --- | --- | --- | --- |
| FS30 | IN_SCOPE | Purchases: history, QR target/retry/missing target, items/amount/date, shop handoff, rating action, loading/empty/error/refresh; refund history placeholder and creation placeholder/action sheet | `settings_view.dart`, `cart_v2_view.dart`, notification destination → `purchases_view.dart` | Pending |
| FS31 | IN_SCOPE | Shop rating history, rated purchases only, rating date fallback/sort, related purchase facts, loading/empty/error/refresh and empty CTA to Purchases | Account → `customer_ratings_view.dart` | Pending |
| MD05 | IN_SCOPE | Shop rating sheet: 1–5 selection, no selection, submitting, failure/retry, success/close and history refresh | Unrated purchase → `_PurchaseShopRatingSheet` in `purchases_view.dart` | Pending |
| FS32 | IN_SCOPE | Product review summary/distribution, own/other cards, verified server projection, guest/eligible/ineligible/previously deleted states, loading/empty/error/refresh | Product details/description → `product_reviews_view.dart` | Pending |
| MD17 | IN_SCOPE | Review create/edit sheet: stars, optional title/comment, validation, keyboard, submitting/error/save | Review eligibility/card → `_ReviewEditorSheet` | Pending |
| MD18 | IN_SCOPE | Review delete confirmation, cancel/delete, delete progress/failure | Own review → confirmation in `product_reviews_view.dart` | Pending |
| FS33 | IN_SCOPE | Inbox: merchant identity, preview/time, unread/read where present, loading/empty/error/refresh and conversation handoff | Account/notification fallback → `conversations_view.dart` | Pending |
| FS34 | IN_SCOPE | Conversation: merchant identity, incoming/outgoing bubbles, dates, actual send/read, composer/limit, pagination/scroll, draft, loading/empty/error and send failure | Shop/profile/seller, pending product intent, inbox, notification → `chat_view.dart` | Pending |
...
| INACTIVE_LEGACY | Legacy order/shipping/detail/refund code without active Customer entry; legacy Address/Store/Orders; MD10 merchant-login compatibility path | Excluded from active Customer denominator; no new route or behavior |
