...
### W51C-R / W51D — Exact signed artifact evidence and main lineage

Date: 2026-09-05 UTC. W51C-R source **a18c6813c45136f863b2dbf7057748486b661b90**,
build source **633f5c91de85c080417557ea51bee344a4a03907**, base
**813f16f54d27b6a25a07cb708fc78244a5e4c791**. Worker reports **13/13** local
input/build/binary/evidence phases, signed APK/AAB and **GREEN / SAME_SIZE**.
The two source paths are pubspec.yaml and its result report. Source-reported
interval **19:45:37–20:32:03.604 UTC = 46m26.604s** includes input/tool waits and
excludes final report/Git delivery. Owner password completion is an input event,
not a product/UI correction. [Source evidence](RELEASE_W51C_SIGNED_RC_CANDIDATE_RESULT.md).

W51D found main exactly equal to merge-base, no newer main commit and no conflict.
All three source commits were reviewed. No-ff merge/checkpoint
**ce99a033647b64eb1256ab75676499e2d294c629** was pushed to
`integration/w51d-signed-rc-evidence`; the evidence commit and final main publication
are identified in delivered TASK_RESULT. Integration adds five Markdown paths,
**7 total paths** from base, with both source files unchanged.

Independent verification preserves both authoritative hashes and the existing
RSA-4096 upload certificate. APK v2 signature PASS; AAB complete JarFile verification
has **548 signed payload entries / 0 unsigned / 0 duplicate names**, all pinned to
the expected signer. AAB self-signed/no-timestamp and stream-view warnings remain
disclosed. Both manifests have **com.esnaftavar.app / 1.0.0 / versionCode 1** and
three ABIs. Twenty excluded samples are absent and three approved Home images are
byte-exact in both packages. No artifact was modified, repacked or generated by W51D.

Targeted **139 PASS / 0 FAIL / 0 SKIP**, analyzer clean (30.8 s), one final full
suite **2065 PASS / 0 FAIL / 6 unchanged skips**, **175/175 files**, runner
**83.716 s**. All **175 old tests / 245 PNGs** remain unchanged. Source net/history
secret/PII scans and tracked sensitive-path checks pass; external secret files
are not opened. Observable Integration start **20:48:29 UTC**, checkpoint boundary
**20:57:01 UTC = 8m32s**, excluding remaining docs/final publication; TASK_RESULT
reports the later boundary.

**Lineage decision:** pubspec.yaml is a binary-affecting input absent from starting
main, although it was already included in artifact build 633f5c9. After that build,
only documentation changed. Therefore **binary delta versus starting main YES**,
**binary delta after build NO**, and, under the user's explicit integration rule,
**rebuild after integration YES**. Existing artifacts are not relabeled final-main
RC. **Device install/launch readiness NO** until a separately authorized final-main
rebuild and exact new artifact proof; no device or Production action is performed.

Six Integration gates cover freshness/delta, artifact proofs, Flutter/analyzer,
safety, lineage/coordination and publication. W51D **YELLOW / SAME_SIZE** retains
the meaningful final-main rebuild requirement despite passing local integration.
No critical regression, scope drift, substantive owner correction or collision
was found. Shared pubspec ownership was reviewed; extra Integration binary/runtime
edits **0**, Figma **NOT_REQUIRED / 0 calls**, backend/Production/store/device
operations **0**. AGENTS.md/protocol unchanged. Recommend one coherent final-main
