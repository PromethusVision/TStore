...
- Free plan scheduled backup/PITR/restorable point sağlamaz. `BACKUP_ROLLBACK_PLAN_READY: NO`, linked CLI dry-run PENDING ve `READY_FOR_PRODUCTION_MIGRATION_APPLY: NO`; accepted RPO/RTO, restore/incident owner, restore drill ve enforced change window kapanmadan apply yapılmaz.
...
- Local migration artifact integrity canonical Git/LF manifestiyle 9/9 ve local safe-equivalent clean-room replay 9/9 PASS; linked CLI Production dry-run ile gerçek apply/postflight kapılarını kapatmaz.
