...
void main() {
  ...
  const favoriteActionKey = Key(
    'vertical-product-card-favorite-product-1-action',
  );
  const favoriteLoadingKey = Key(
    'vertical-product-card-favorite-product-1-loading',
  );
  ...
  Widget buildSubject({
    ...
    bool favoriteActionLoading = false,
  ...
  testWidgets('ürünü kart üzerindeki boş kalpten favorilere ekler', (
    tester,
  ) async {
  ...
  testWidgets(
    'favoriler hazırlanırken kalp yerine yükleme göstergesini sunar',
    (tester) async {
  ...
  testWidgets('Favorilerim ekranındaki doğrudan kaldırma davranışını korur', (
    ...
    await tester.pumpWidget(
      buildSubject(
        initialState: WishlistLoaded(const [wishlistItem]),
        onFavoritePressed: () => pressed = true,
      ),
    );
    await tester.pumpAndSettle();
